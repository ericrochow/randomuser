...
///   K1 and K2 must not both be 0; retry if last digit is 0.
fn gen_fnr(birth_date_str: &str, birth_year: i32, gender: &str, prng: &mut Prng) -> String {
    loop {
        let no = {
            ...
            let mut n = (hi - prng.range(0, hi - lo)) as i64;
        ...
        let fnr = format!("{}{}{}{}", birth_date_str, pad(no, 3), k1, k2);
        if fnr.len() != 11 || fnr.ends_with('0') {
            continue;
        }
        return fnr;
    }
        ...
        if inc.iter().any(|f| f == "id") {
            ...
            if let Some(dob) = dob_date {
                ...
                let gender = user
                    ...
                    .and_then(|g| g.as_str())
                    .unwrap_or("male");
                let fnr = gen_fnr(&birth_date_str, dob.year(), gender, prng);
...
mod tests {
    ...
    #[test]
    fn fnr_does_not_end_with_zero() {
        let mut prng = Prng::new();
        prng.seed_from_str("no_fnr_zero", 1);
        for _ in 0..20 {
            let fnr = gen_fnr("010185", 1985, "female", &mut prng);
            assert!(!fnr.ends_with('0'), "FNR must not end with 0: {fnr}");
        }
    }
